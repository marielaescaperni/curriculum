...
<a
	...
	class={`group relative isolate flex h-full min-h-[680px] w-full flex-col overflow-hidden rounded-[var(--radius-panel)] border border-white/70 bg-white/50 text-left shadow-[0_18px_60px_rgba(0,0,0,0.05)] backdrop-blur-md transition duration-500 hover:-translate-y-1 hover:rotate-[-0.4deg] hover:border-transparent hover:bg-white/75 hover:shadow-[0_24px_80px_rgba(0,0,0,0.10)] md:min-h-[720px] ${className}`}
>
	<div class="pointer-events-none absolute -right-20 -top-20 z-0 h-56 w-56 rounded-full bg-[var(--project-accent)] opacity-0 blur-3xl transition duration-500 group-hover:opacity-35"></div>
	...
	<div class="relative z-[2] h-[240px] shrink-0 overflow-hidden bg-neutral-100">
		{#if project.thumbnail}
			...
			/>
		{:else}
			<div class="flex h-full w-full items-end bg-[linear-gradient(135deg,var(--project-accent),var(--gradient-purple),var(--gradient-blue))] p-6">
				<p class="max-w-sm font-display text-5xl font-semibold leading-none tracking-[-0.06em] text-white">{project.title}</p>
			</div>
		{/if}
		...
		<div class="absolute left-5 top-5 flex flex-wrap items-center gap-2">
			<span class="rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-white bg-main-gradient">0{index}</span>
			<span class="rounded-full border px-3 py-1 text-xs font-medium text-white bg-main-gradient">{project.period}</span>
		</div>
	</div>

	<div class="relative z-[2] flex flex-1 flex-col p-6 md:p-7">
		<p class="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--color-text-muted)]">{project.subtitle}</p>
		...
		<div class="mt-auto pt-7">
			<div class="flex flex-wrap gap-2">
				{#each project.tags.slice(0, 4) as tag}
					<span class="rounded-full border border-white/70 bg-white/60 px-3 py-1.5 text-sm font-medium text-[var(--color-text-secondary)] shadow-sm backdrop-blur-md transition duration-300 group-hover:bg-white/80">{tag}</span>
				{/each}
			</div>
