...
>
	...
	<div class="relative z-[2] h-[240px] shrink-0 overflow-hidden bg-neutral-100">
		{#if project.thumbnail}
			<img
				...
				class="h-full w-full scale-[1.03] object-cover object-top transition duration-1000 ease-out"
			/>
		{:else}
			<div
		...
		<div class="absolute left-5 top-5 flex flex-wrap items-center gap-2">
			...
			<span class="rounded-full border px-3 py-1 text-xs font-medium text-white bg-main-gradient">
				{project.period}
			</span>
		</div>
	</div>
	...
	<div class="relative z-[2] flex flex-1 flex-col p-6 md:p-7">
		...
		<div class="mt-4 flex items-start justify-between gap-6">
			<h3
				class="line-clamp-2 font-display text-4xl font-semibold leading-none tracking-[-0.05em] text-[var(--color-text-primary)] md:text-5xl"
			>
				{project.title}
			</h3>
		...
		<p class="mt-5 line-clamp-4 text-base leading-relaxed text-[var(--color-text-secondary)]">
			{project.description}
		</p>
