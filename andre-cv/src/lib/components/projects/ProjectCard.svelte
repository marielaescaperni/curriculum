...
<button
	...
	class={`group relative overflow-hidden rounded-[var(--radius-panel)] border border-white/70 bg-white/50 text-left shadow-[0_18px_60px_rgba(0,0,0,0.05)] backdrop-blur-md transition duration-500 hover:-translate-y-1 hover:rotate-[-0.4deg]  hover:border-transparent hover:bg-white/75 hover:shadow-[0_24px_80px_rgba(0,0,0,0.10)] ${className}`}
>
	<div
		class="pointer-events-none absolute -right-20 -top-20 h-56 w-56 rounded-full bg-[var(--project-accent)] opacity-0 blur-3xl transition duration-500 group-hover:opacity-35"
	></div>
	<div
...
></div>
	...
	<div class="relative aspect-[16/10] overflow-hidden bg-neutral-100">
		{#if project.thumbnail}
			<img
				...
				class="h-full w-full object-cover transition duration-1000 ease-out group-hover:scale-[1.04]"
			/>
		{:else}
			<div
		...
		<div class="absolute left-5 top-5 flex flex-wrap items-center gap-2">
			<span
				class="rounded-full border border-white/50 bg-white/30 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-white backdrop-blur-md"
			>
				0{index}
			</span>

			<span
				class="rounded-full border border-white/50 bg-white/30 px-3 py-1 text-xs font-medium text-white backdrop-blur-md"
			>
				{project.period}
			</span>
		</div>
	...
	<div class="relative z-10 p-6 md:p-7">
		...
		<p
			class="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-[var(--color-text-primary)]"
		>
			View case study
